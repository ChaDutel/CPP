...
/*   Updated: 2023/05/13 18:59:05 by cdutel-l         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	printResult(char c, int i, float f, double d)
{
	...
	// if ((c >= 0 && c <= 31) || (d > 2147483647 || d < -2147483648))
	// {
	// 	if (d > 2147483647 || d < -2147483648)
	// 		std::cout << std::fixed << std::setprecision(1) << "char: impossible" << "\nint: impossible" << "\nfloat: " << f << "f\ndouble: " << d << std::endl;
	// 	else if (((c >= 0 && c <= 31) || c == 127) && ((d >= 0 && d <= 31) || d > 126))
	// 		std::cout << std::fixed << std::setprecision(1) << "char: Non displayable" << "\nint: " << i << "\nfloat: " << f << "f\ndouble: " << d << std::endl;
	// 	std::cout << d << std::endl;
	// }
	// else
	...
	// else if (d > 2147483647.0 || d < -2147483648)
	// 	std::cout << "char: '" << c << "'\nint: impossible" << "\nfloat: " << f << "f\ndouble: " << d << std::endl;
	// else
	// 	std::cout << "char: '" << c << "'\nint: " << i << "\nfloat: " << std::setprecision(1) << f << "f\ndouble: " << d << std::endl;
}
...
int	isDouble(char *str)
{
	...
	while (str[i])
	{
		if ((str[i] < 48 || str[i] > 57) && (str[i] != 43 || str[i] != 45 || str[i] != 46))
			return (-1);
		else if (i != 0 && (str[i] == 43 || str[i] == 45))
			return (-1);
		else if (str[i] == 46)
...
int	isFloat(char *str)
{
	...
	while (str[i])
	{
		if ((str[i] < 48 || str[i] > 57) && (str[i] != 43 || str[i] != 45 || str[i] != 46 || str[i] != 102))
			return (-1);
		else if (i != 0 && (str[i] == 43 || str[i] == 45))
			return (-1);
		else if (str[i] == 46)
...
int    main(int argc, char **argv)
{
	...
	switch (identifiedType)
	{
		...
		case 3 :
			...
			break;
		case 4 :
			std::cout << identifiedType << std::endl;
			castDouble(argv[1]);
			break;
		case 5 :
			castChar(argv[1]);
